...
namespace EletroCarConsole
{
    ...
    class VistoriaDto
    {
        ...
        [JsonPropertyName("motivoReprovacao")] public string MotivoReprovacao { get; set; }
    ...
    {
        // Mesma URL do frontend (EletroCarAPI/appsettings.json)
        ...
        {
            var resp = await Http.GetAsync(BuildUrl(path));
            var json = await resp.Content.ReadAsStringAsync();
            if (!resp.IsSuccessStatusCode)
                throw new HttpRequestException($"HTTP {(int)resp.StatusCode}: {json}");
        ...
        public static async Task<T> Post<T>(string path, object body)
        {
            ...
            var resp = await Http.PostAsync(BuildUrl(path), content);
            var json = await resp.Content.ReadAsStringAsync();
            if (!resp.IsSuccessStatusCode)
                throw new HttpRequestException($"HTTP {(int)resp.StatusCode}: {json}");
            return JsonSerializer.Deserialize<T>(json, JsonOpts);
        ...
        public static async Task<bool> Ping()
        {
            try
            {
                var r = await Http.GetAsync(BuildUrl("Veiculo"));
    ...
    class Program
    {
        ...
        {
            LimparTela();
            Console.WriteLine(">>> VISTORIAS (PENDENTES E HISTÓRICO) <<<");
            Console.WriteLine("========================================\n");
            try
            {
                var lista = await ApiService.Get<List<VistoriaDto>>("Vistoria/todas");

                if (lista == null || lista.Count == 0)
                {
                    Console.WriteLine("Nenhuma vistoria registrada.");
                    Pausar(); return;
                }

                var pendentes = lista.Count(v => string.Equals(v.Status, "Pendente", StringComparison.OrdinalIgnoreCase));
                Console.WriteLine($"Pendentes: {pendentes} | Total no histórico: {lista.Count}\n");

                Console.WriteLine($"{"ID",-6} {"Reserva",-10} {"Veículo",-22} {"Cliente",-22} {"Data",-18} {"Status",-14} {"Motivo/Obs",-20}");
                Console.WriteLine(new string('-', 120));
                foreach (var v in lista)
                {
                    ...
                    string motivo = !string.IsNullOrWhiteSpace(v.MotivoReprovacao) ? v.MotivoReprovacao
                        : (v.Observacoes ?? "-");
                    if (motivo.Length > 18) motivo = motivo[..18] + "...";
                    string st = v.Status switch
                    {
                        "Aprovada"  => "✅ Aprovada",
                        "Reprovada" => "❌ Reprovada",
                    ...
                    Console.WriteLine($"{v.Id,-6} #{v.ReservaId,-9} {modelo,-22} {cliente,-22} {v.DataSolicitacao:dd/MM/yyyy HH:mm} {st,-14} {motivo,-20}");
                }

                Console.Write("\nDigite o ID da vistoria PENDENTE para analisar (0=sair): ");
                ...
                if (!string.Equals(vistoria.Status, "Pendente", StringComparison.OrdinalIgnoreCase))
                {
                    Console.WriteLine($"Esta vistoria já foi finalizada ({vistoria.Status}).");
                    Pausar(); return;
                }
                ...
                var atualizado = await ApiService.Get<List<VistoriaDto>>("Vistoria/todas");
                var vAtual = atualizado?.FirstOrDefault(x => x.Id == vid);
                if (vAtual != null)
                    Console.WriteLine($"\nRegistro no banco: Status = {vAtual.Status}" +
                        (string.IsNullOrWhiteSpace(vAtual.MotivoReprovacao) ? "" : $" | Motivo: {vAtual.MotivoReprovacao}"));
            }
            catch (Exception ex)
            {
                Console.WriteLine($"❌ Erro: {ex.Message}");
